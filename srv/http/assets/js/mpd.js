...
$( '#audiooutput' ).on( 'selectric-change', function() {
	...
	sh( [ mpdsh, 'audiooutput', G.audioaplayname, card, G.audiooutput, hwmixer ], refreshData );
...
$( '#setting-mixertype' ).click( function() { // hardware mixer
	...
	sh( [ mpdsh, 'amixer', card ], function( data ) {
		...
		info( {
			...
			, ok      : function() {
				...
				sh( [ mpdsh, 'mixerhw', name, mixer, mixermanual, card ], refreshData );
			}
		} );
	}, 'json' );
} );
	...
	if ( checked ) {
		info( {
			...
			, ok      : function() {
				...
				sh( [ mpdsh, 'novolume', name ], refreshData );
			}
		} );
	} else {
		info( {
...
$( '#dop' ).click( function() {
	...
	sh( [ mpdsh, 'dop', checked, name ], refreshData );
} );
$( '#crossfade' ).click( function() {
	if ( $( this ).prop( 'checked' ) ) {
		...
		sh( [ mpdsh, 'crossfade' ], refreshData );
	}
} );
	...
	info( {
		...
		, ok      : function() {
			...
			if ( crossfade !== G.crossfade ) {
				...
				sh( [ mpdsh, 'crossfade ', G.crossfade ], refreshData );
			}
		}
	} );
} );
$( '#normalization' ).click( function() {
	...
	sh( [ mpdsh, 'normalization', G.normalization ], refreshData );
} );
$( '#replaygain' ).click( function() {
	if ( $( this ).prop( 'checked' ) ) {
		...
		sh( [ mpdsh, 'replaygain' ], refreshData );
	}
} );
	...
	info( {
		...
		, ok        : function() {
			...
			if ( replaygain !== G.replaygain ) {
				...
				sh( [ mpdsh, 'replaygain', G.replaygain ], refreshData );
			}
		}
	} );
} );
$( '#autoupdate' ).click( function() {
	...
	sh( [ mpdsh, 'autoupdate', G.autoupdate ], refreshData );
} );
	...
	info( {
		...
		, ok        : function() {
			...
			if ( buffer < 4097 ) {
				...
				sh( [ mpdsh, 'buffer', G.buffer ], refreshData );
			}
		}
	} );
} );
$( '#ffmpeg' ).click( function() {
	...
	sh( [ mpdsh, 'ffmpeg', G.ffmpeg ], refreshData );
} );
...
function getStatus() {
	sh( [ mpdsh, 'statusmpd' ], function( status ) {
		$( '#codestatus' )
			.html( status )
			.removeClass( 'hide' );
	} );
...
function setMixerType( mixertype ) {
	...
	sh( [ mpdsh, 'mixerset', mixertype, name, card, hwmixer ], refreshData );
}
