...
$( '#setting-upnp' ).click( function() {
	info( {
		  icon     : 'upnp'
		, title    : 'UPnP Renderer'
		, content  : htmlgmusic + htmlqobuz + htmlspotify + htmltidal
		, preshow  : function() {
			...
			$( '.infotextbox' ).on( 'click', '.enable', function( e ) {
		...
		, ok       : function() {
			var conf = '';
			var cmd = [
				  "sed -i '/.*user =/,$ d' /etc/upmpdcli.conf"
				, 'rm -f '+ dirsystem +'/upnp-*'
			];
			var value = {};
			[ 'tidal', 'qobuz', 'gmusic', 'spotify' ].forEach( function( service ) {
				...
				if ( $( '#'+ service +' input' ).prop( 'checked' ) ) {
					if ( user && pass ) {
						...
						conf += serviceuser +' = '+ user +'\n'
							  + servicepass +' = '+ pass + +'\n';
						if ( service === 'qobuz' ) {
							conf += 'qobuzformatid = '+ quality +'\n';
						} else if ( service === 'gmusic' || service === 'tidal' ) {
							conf += servicequal +' = '+ quality +'\n';
						}
						cmd.push(
